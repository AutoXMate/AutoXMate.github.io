...
NEW_TOOLS = [
    ...
    ("uname", "uname", "system:linux:info", "system",
     "Prints system information like kernel name, hostname, OS, and architecture.",
     None, ["linux"], []),
    ("uptime", "uptime", "system:linux:info", "system",
     "Shows how long the system has been running, user count, and load averages.",
     None, ["linux"], []),
    ("who", "who", "system:linux:user", "system",
     "Displays who is logged in and their session details.",
     None, ["linux"], []),
    ("lastlog", "lastlog", "system:linux:auth", "system",
     "Reports the most recent login of all users or a given user.",
     None, ["linux"], []),
    ("lscpu", "lscpu", "system:linux:cpu", "system",
     "Displays CPU architecture information like cores, threads, model, and flags.",
     None, ["linux"], []),
    ("lsblk", "lsblk", "system:linux:disk", "system",
     "Lists block devices (disks and partitions) in a tree-like format.",
     None, ["linux"], []),
    ("lspci", "lspci", "system:linux:pci", "system",
     "Lists all PCI devices connected to the system.",
     None, ["linux"], []),
    ("lsusb", "lsusb", "system:linux:usb", "system",
     "Lists USB buses and connected USB devices.",
     None, ["linux"], []),
    ("mountpoint", "mountpoint", "system:linux:filesystem", "system",
     "Checks if a directory is a mount point.",
     None, ["linux"], []),
    ("findmnt", "findmnt", "system:linux:filesystem", "system",
     "Lists all currently mounted filesystems in a tree format.",
     None, ["linux"], []),
    ("vmstat", "vmstat", "system:linux:monitor", "system",
     "Reports virtual memory statistics, processes, CPU, and I/O activity.",
     None, ["linux"], []),
    ("iostat", "iostat", "system:linux:monitor", "system",
     "Reports CPU and I/O statistics for devices and partitions.",
     None, ["linux"], []),
    ("nproc", "nproc", "system:linux:cpu", "system",
     "Prints the number of processing units available.",
     None, ["linux"], []),
    ("locale", "locale", "system:linux:config", "system",
     "Displays locale-related environment variables and settings.",
     None, ["linux"], []),
    ("lsmod", "lsmod", "system:linux:kernel", "system",
     "Lists loaded kernel modules.",
     None, ["linux"], []),
    ("modprobe", "modprobe", "system:linux:kernel", "system",
     "Adds or removes kernel modules from the Linux kernel.",
     None, ["linux"], []),
    ("modinfo", "modinfo", "system:linux:kernel", "system",
     "Displays information about a kernel module.",
     None, ["linux"], []),
    ("depmod", "depmod", "system:linux:kernel", "system",
     "Generates modules.dep and map files for kernel modules.",
     None, ["linux"], []),
    ("blkid", "blkid", "system:linux:disk", "system",
     "Locates/prints block device attributes like UUID and filesystem type.",
     None, ["linux"], []),
    ("eject", "eject", "system:linux:device", "system",
     "Ejects removable media (CD-ROM, USB, etc.) from the system.",
     None, ["linux"], []),
     
    # Network/remote tools
    ("ldapwhoami", "ldapwhoami", "security:ldap:query", "security",
     "Sends an LDAP WhoAmI extended operation to a directory server.",
     "custom", ["linux"], []),
    ("showmount", "showmount", "network:nfs:export", "network",
     "Shows NFS server export lists and mount information.",
     "custom", ["linux"], []),
    ("nfsstat", "nfsstat", "network:nfs:stats", "network",
     "Displays NFS statistics about mounts, RPC calls, and server activity.",
     "custom", ["linux"], []),
    
    # Cloud clients
    ("az", "az", "system:cloud:azure", "system",
     "Azure CLI - manages Azure resources from the command line.",
     "custom", ["linux"], []),
    
    # Database clients
    ("psql", "psql", "system:database:postgresql", "system",
     "PostgreSQL interactive terminal and query tool.",
     "custom", ["linux"], []),
    ("redis-cli", "redis-cli", "system:database:redis", "system",
     "Redis command line client for querying and managing Redis instances.",
     "custom", ["linux"], []),
    ("mongosh", "mongosh", "system:database:mongodb", "system",
     "MongoDB Shell — interactive JavaScript interface to MongoDB.",
     "custom", ["linux"], []),
]
...
INSTALL_CMDS = {
    ...
    "blkid": None,
    "eject": None,
    "ldapwhoami": "apt-get install -y ldap-utils",
    "showmount": "apt-get install -y nfs-common",
    "nfsstat": "apt-get install -y nfs-common",
...
def generated(entry):
    ...
    if "-" not in fm_id:
        ...
        prefix = prefix_map.get(domain, "cmd")
