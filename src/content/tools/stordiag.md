...
name: stordiag
description: 'Storage diagnostic tool Located at: c:\windows\system32\stordiag.exe;
...
- description: Once executed, Stordiag.exe will execute schtasks.exe systeminfo.exe
    ...
    is renamed to one of these names, stordiag.exe will execute it. (Possible defence
    evasion purposes.)
...
- description: Once executed, Stordiag.exe will execute schtasks.exe and powershell.exe
    ...
    to one of these names, stordiag.exe will execute it. (Possible defence evasion
    purposes.)
  command: stordiag.exe
references:
...
- type: ioc
  description: systeminfo.exe, fltmc.exe or schtasks.exe or powershell.exe being executed
