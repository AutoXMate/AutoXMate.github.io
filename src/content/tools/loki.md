...
name: loki
description: IOC (Indicator of Compromise) scanner that checks files and systems against
  threat intelligence feeds and YARA rules for malware detection.
...
- name: flag-verbose
  ...
  - --verbose
execution:
  template: python3 loki.py {flag-yara} {flag-rules} {flag-ioc} {flag-score} {flag-output}
    {flag-format} {flag-nobeeps} {flag-no-progress} {flag-verbose} -p {path}
  sandbox: execFile
  timeout_seconds: 600
  shell: false
examples:
