...
description: The first in-the-wild UEFI bootkit bypassing UEFI Secure Boot on fully
  updated UEFI systems is now a reality. Once the persistence is configured, the BlackLotus
...
execution:
  template: sc.exe create blacklotus_driver.sys binPath=C:\windows\temp\blacklotus_driver.sys
...
- method: custom
  ...
  commands:
  - sc.exe create blacklotus_driver.sys binPath=C:\windows\temp\blacklotus_driver.sys
    type=kernel && sc.exe start blacklotus_driver.sys
references:
- label: Reference
