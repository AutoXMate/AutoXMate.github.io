...
name: MemCtl.sys
description: MemCtl.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create MemCtl binPath=C:\windows\temp\MemCtl.sys type=kernel &&
    sc.exe start MemCtl
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create MemCtl binPath=C:\windows\temp\MemCtl.sys type=kernel && sc.exe
    start MemCtl
references:
- label: Reference
