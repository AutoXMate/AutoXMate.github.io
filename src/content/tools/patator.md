...
name: patator
description: Multi-protocol brute-forcing tool that supports FTP, SSH, HTTP, SMTP,
  SNMP, and many other services for login credential testing.
...
- name: flag-output
  ...
  - --output
execution:
  template: patator {module} {flag-hosts} {flag-users} {flag-passwords} {flag-port}
    {flag-threads} {flag-timeout} {flag-max-retries} {flag-output}
  sandbox: execFile
  timeout_seconds: 600
  shell: false
examples:
...
- description: HTTP form brute-force
  command: patator http_fuzz url=http://example.com/login method=POST body='user=FILE0&pass=FILE1'
