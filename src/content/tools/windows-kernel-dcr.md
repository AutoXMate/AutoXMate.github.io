...
execution:
  template: sc.exe create dcr.sys binPath=C:\windows\temp\dcr.sys type=kernel && sc.exe
    start dcr.sys
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create dcr.sys binPath=C:\windows\temp\dcr.sys type=kernel && sc.exe start
    dcr.sys
references:
- label: Reference
