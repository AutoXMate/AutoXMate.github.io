...
name: watabe.sys
description: watabe.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create watabe binPath=C:\windows\temp\watabe.sys type=kernel &&
    sc.exe start watabe
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create watabe binPath=C:\windows\temp\watabe.sys type=kernel && sc.exe
    start watabe
references:
- label: Reference
