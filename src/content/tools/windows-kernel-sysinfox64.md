...
name: SysInfoX64.sys
description: SysInfoX64.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create SysInfoX64 binPath=C:\windows\temp\SysInfoX64.sys type=kernel
    && sc.exe start SysInfoX64
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create SysInfoX64 binPath=C:\windows\temp\SysInfoX64.sys type=kernel &&
    sc.exe start SysInfoX64
references:
- label: Reference
