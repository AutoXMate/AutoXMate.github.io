...
name: tplmap
description: Server-Side Template Injection (SSTI) exploitation tool that automates
  detection and exploitation of template injection vulnerabilities across multiple
  template engines.
...
- name: flag-proxy
  ...
  - --proxy
execution:
  template: tplmap {flag-data} {flag-cookie} {flag-user-agent} {flag-header} {flag-engine}
    {flag-os-shell} {flag-os-cmd} {flag-read-file} {flag-level} {flag-proxy} -u {url}
  sandbox: execFile
  timeout_seconds: 300
  shell: false
examples:
