...
name: lfi-suite
description: Local File Inclusion exploitation suite that automates LFI detection,
  file reading, RCE via log poisoning, and PHP wrapper attacks.
...
- name: flag-proxy
  ...
  - --proxy
execution:
  template: python3 lfi.py {flag-type} {flag-os-cmd} {flag-read-file} {flag-shell}
    {flag-proxy} -t {target}
  sandbox: execFile
  timeout_seconds: 120
  shell: false
