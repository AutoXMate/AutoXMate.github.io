...
execution:
  template: sc.exe create EIO.sys binPath=C:\windows\temp\EIO.sys type=kernel && sc.exe
    start EIO.sys
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create EIO.sys binPath=C:\windows\temp\EIO.sys type=kernel && sc.exe start
    EIO.sys
detections:
- type: yara
