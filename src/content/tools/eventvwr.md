...
name: eventvwr
description: 'Displays Windows Event Logs in a GUI window. Located at: C:\Windows\System32\eventvwr.exe;
...
- description: During startup, eventvwr.exe checks the registry value `HKCU\Software\Classes\mscfile\shell\open\command`
    ...
    to the user. (Execute a binary or script as a high-integrity process without a
    UAC prompt.)
...
- description: During startup, eventvwr.exe uses .NET deserialization with `%LOCALAPPDATA%\Microsoft\EventV~1\RecentViews`
    ...
    (Execute a command to bypass security restrictions that limit the use of command-line
    interpreters.)
