...
execution:
  template: sc.exe create ACPIx86.sys binPath=C:\windows\temp\ACPIx86.sys type=kernel
    && sc.exe start ACPIx86.sys
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create ACPIx86.sys binPath=C:\windows\temp\ACPIx86.sys type=kernel && sc.exe
    start ACPIx86.sys
references:
- label: Reference
