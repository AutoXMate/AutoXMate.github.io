...
description: HardwareMon-x86.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create HardwareMon-x86 binPath=C:\windows\temp\HardwareMon-x86.sys
    type=kernel && sc.exe start HardwareMon-x86
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create HardwareMon-x86 binPath=C:\windows\temp\HardwareMon-x86.sys type=kernel
    && sc.exe start HardwareMon-x86
references:
- label: Reference
