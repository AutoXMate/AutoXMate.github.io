...
name: whql.sys
description: whql.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create whql binPath=C:\windows\temp\whql.sys type=kernel && sc.exe
    start whql
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create whql binPath=C:\windows\temp\whql.sys type=kernel && sc.exe start
    whql
references:
- label: Reference
