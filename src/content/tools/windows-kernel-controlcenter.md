...
description: ControlCenter.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create ControlCenter binPath=C:\windows\temp\ControlCenter.sys
    type=kernel && sc.exe start ControlCenter
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create ControlCenter binPath=C:\windows\temp\ControlCenter.sys type=kernel
    && sc.exe start ControlCenter
references:
- label: Reference
