...
execution:
  template: sc.exe create LHA.sys binPath=C:\windows\temp\LHA.sys type=kernel && sc.exe
    start LHA.sys
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create LHA.sys binPath=C:\windows\temp\LHA.sys type=kernel && sc.exe start
    LHA.sys
detections:
- type: yara
