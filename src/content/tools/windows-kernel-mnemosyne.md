...
name: Mnemosyne.sys
description: Mnemosyne.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create Mnemosyne binPath=C:\windows\temp\Mnemosyne.sys type=kernel
    && sc.exe start Mnemosyne
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create Mnemosyne binPath=C:\windows\temp\Mnemosyne.sys type=kernel && sc.exe
    start Mnemosyne
references:
- label: Reference
