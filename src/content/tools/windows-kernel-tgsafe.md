...
execution:
  template: sc.exe create TGSafe.sys binPath=C:\windows\temp\TGSafe.sys type=kernel
...
- method: custom
  ...
  commands:
  - sc.exe create TGSafe.sys binPath=C:\windows\temp\TGSafe.sys type=kernel && sc.exe
    start TGSafe.sys
references:
- label: Reference
