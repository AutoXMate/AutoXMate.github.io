...
name: dns-sd
description: Discover local network services using the DNS-Based Service Discovery
  (SD) protocol.
...
execution:
  ...
  shell: false
examples:
- description: 'Discover SSH hosts: Hosts serving SSH can be discovered using the
    _ssh._tcp service string.'
  command: dns-sd -B _ssh._tcp
- description: 'Discover web hosts: Hosts serving web services can be discovered using
    the _http._tcp service string.'
  command: dns-sd -B _http._tcp
- description: 'Discover hosts serving remote screen sharing: Hosts serving remote
    screen sharing can be discovered using the _rfb._tcp service string.'
  command: dns-sd -B _rfb._tcp
- description: 'Discover hosts serving SMB: Hosts serving SMB can be discovered using
    the _smb._tcp service string.'
