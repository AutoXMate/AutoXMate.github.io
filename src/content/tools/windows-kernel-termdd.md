...
execution:
  template: sc.exe create termdd.sys binPath=C:\windows\temp\termdd.sys type=kernel
...
- method: custom
  ...
  commands:
  - sc.exe create termdd.sys binPath=C:\windows\temp\termdd.sys type=kernel && sc.exe
    start termdd.sys
references:
- label: Reference
