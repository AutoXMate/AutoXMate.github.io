...
name: arjun
description: HTTP parameter discovery tool that finds hidden GET/POST parameters by
...
- description: Scan with custom wordlist and headers
  command: 'arjun -u http://target.com/api -w custom_params.txt --headers ''{"X-Custom":
    "value"}'''
references:
- label: Arjun GitHub
