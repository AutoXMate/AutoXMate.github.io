...
description: bin_intigua_driver64.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create bin_intigua_driver64 binPath=C:\windows\temp\bin_intigua_driver64.sys
    type=kernel && sc.exe start bin_intigua_driver64
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create bin_intigua_driver64 binPath=C:\windows\temp\bin_intigua_driver64.sys
    type=kernel && sc.exe start bin_intigua_driver64
references:
- label: Reference
