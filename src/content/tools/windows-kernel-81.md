...
execution:
  template: sc.exe create 81.sys binPath=C:\windows\temp\81.sys type=kernel && sc.exe
    start 81.sys
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create 81.sys binPath=C:\windows\temp\81.sys type=kernel && sc.exe start
    81.sys
references:
- label: Reference
