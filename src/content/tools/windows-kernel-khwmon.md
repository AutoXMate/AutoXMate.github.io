...
name: Khwmon.sys
description: Khwmon.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create Khwmon binPath=C:\windows\temp\Khwmon.sys type=kernel &&
    sc.exe start Khwmon
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create Khwmon binPath=C:\windows\temp\Khwmon.sys type=kernel && sc.exe
    start Khwmon
references:
- label: Reference
