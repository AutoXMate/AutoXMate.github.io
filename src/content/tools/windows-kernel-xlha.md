...
description: XLHA.sys is a kernel driver from LG Electronics Inc. that exposes physical
  memory read/write via MmMapIoSpace and MSR read. Two other LG LHA.sys variants are
...
execution:
  template: sc.exe create XLHA binPath=C:\windows\temp\XLHA.sys type=kernel && sc.exe
    start XLHA
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create XLHA binPath=C:\windows\temp\XLHA.sys type=kernel && sc.exe start
    XLHA
references:
- label: Reference
