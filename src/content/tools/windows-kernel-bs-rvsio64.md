...
name: BS_RVSIO64.sys
description: BS_RVSIO64.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create BS_RVSIO64 binPath=C:\windows\temp\BS_RVSIO64.sys type=kernel
    && sc.exe start BS_RVSIO64
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create BS_RVSIO64 binPath=C:\windows\temp\BS_RVSIO64.sys type=kernel &&
    sc.exe start BS_RVSIO64
references:
- label: Reference
