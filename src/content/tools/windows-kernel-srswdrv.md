...
name: srswdrv.sys
description: srswdrv.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create srswdrv binPath=C:\windows\temp\srswdrv.sys type=kernel
    && sc.exe start srswdrv
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create srswdrv binPath=C:\windows\temp\srswdrv.sys type=kernel && sc.exe
    start srswdrv
references:
- label: Reference
