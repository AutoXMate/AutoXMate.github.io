...
name: NSecKrnl.sys
description: Driver used by ValleyRAT malware to terminate security processes via
  IOCTL 0x2248E0
author: Michael Haag
version: 1.0.0
capabilities:
...
execution:
  template: sc.exe create NSecKrnl binPath=C:\windows\temp\NSecKrnl.sys type=kernel
    && sc.exe start NSecKrnl
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create NSecKrnl binPath=C:\windows\temp\NSecKrnl.sys type=kernel && sc.exe
    start NSecKrnl
detections:
- type: yara
