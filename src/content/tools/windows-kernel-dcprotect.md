...
name: DcProtect.sys
description: bundled with chinese application "DrvCeo" is a set of rootkits. The malicious
  functionality. prevents registry value writing where the registry key or value includes
  "dcprotect" or "drvceo". Prevents file deletion if pathname contains "driverdownload",
  "program files\sysceo", "program files (x86)\sysceo"
author: Wack0
version: 1.0.0
capabilities:
...
execution:
  template: sc.exe create DcProtect.sys binPath=C:\windows\temp\DcProtect.sys type=kernel
...
- method: custom
  ...
  commands:
  - sc.exe create DcProtect.sys binPath=C:\windows\temp\DcProtect.sys type=kernel
    && sc.exe start DcProtect.sys
references:
- label: Reference
