...
name: OAToolx64.sys
description: OAToolx64.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create OAToolx64 binPath=C:\windows\temp\OAToolx64.sys type=kernel
    && sc.exe start OAToolx64
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create OAToolx64 binPath=C:\windows\temp\OAToolx64.sys type=kernel && sc.exe
    start OAToolx64
references:
- label: Reference
