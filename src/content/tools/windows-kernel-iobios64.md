...
name: iobios64.sys
description: iobios64.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create iobios64 binPath=C:\windows\temp\iobios64.sys type=kernel
    && sc.exe start iobios64
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create iobios64 binPath=C:\windows\temp\iobios64.sys type=kernel && sc.exe
    start iobios64
references:
- label: Reference
