...
description: 'msedgewebview2.exe is the executable file for Microsoft Edge WebView2,
  which is a web browser control used by applications to display web content. Located
  at: C:\Program Files (x86)\Microsoft\Edge\Application\114.0.1823.43\msedgewebview2.exe;
...
- description: This command launches the Microsoft Edge WebView2 browser control without
    sandboxing and will spawn the specified executable as its subprocess. (Proxy execution
    of binary)
  command: msedgewebview2.exe --no-sandbox --browser-subprocess-path="{PATH_ABSOLUTE:.exe}"
- description: This command launches the Microsoft Edge WebView2 browser control without
    sandboxing and will spawn the specified command as its subprocess. (Proxy execution
    of binary)
...
- type: ioc
  description: 'msedgewebview2.exe spawned with any of the following: --gpu-launcher,
    --utility-cmd-prefix, --renderer-cmd-prefix, --browser-subprocess-path'
install:
- method: choco
