---
id: windows-kernel-aoddriver
namespace: windows:kernel:aoddriver
name: AODDriver.sys
...
execution:
  template: sc.exe create AODDriversys binPath= C:\windows\temp\AODDriversys.sys type=kernel
...
install:
- method: custom
  description: Load AODDriver.sys kernel driver
  commands:
  - sc.exe create AODDriversys binPath= C:\windows\temp\AODDriversys.sys type=kernel
    && sc.exe start AODDriversys
detections:
- type: yara
...
examples:
  - description: "Load the kernel driver"
    command: "sc.exe create AODDriversys binPath= C:\\\\windows\\\\temp\\\\AODDriversys.sys type=kernel && sc.exe start AODDriversys"
  - description: "Exploit the driver for privilege escalation"
    command: "Exploit.exe --driver AODDriver.sys"
