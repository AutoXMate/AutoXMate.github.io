...
name: gibepext.sys
description: gibepext.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create gibepext binPath=C:\windows\temp\gibepext.sys type=kernel
    && sc.exe start gibepext
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create gibepext binPath=C:\windows\temp\gibepext.sys type=kernel && sc.exe
    start gibepext
references:
- label: Reference
