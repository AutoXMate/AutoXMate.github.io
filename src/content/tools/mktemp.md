...
name: mktemp
description: Create a temporary file or directory and return the file/directory name
  to stdout
...
execution:
  ...
  shell: false
examples:
- description: 'Generate payload directory (Shlayer): The following command can be
    used to generate a random directory name for staging payloads'
  command: export tmpDir="$(mktemp -d /tmp/XXXXXXXXXXXX)"
- description: 'Generate directory based on template file (Bundlore): The following
    command can be used to generate a unique directory based on a template'
