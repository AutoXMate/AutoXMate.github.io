...
execution:
  template: sc.exe create amp.sys binPath=C:\windows\temp\amp.sys type=kernel && sc.exe
    start amp.sys
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create amp.sys binPath=C:\windows\temp\amp.sys type=kernel && sc.exe start
    amp.sys
detections:
- type: yara
