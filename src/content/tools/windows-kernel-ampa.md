...
execution:
  template: sc.exe create ampa binPath=C:\windows\temp\ampa.sys type=kernel && sc.exe
    start ampa
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create ampa binPath=C:\windows\temp\ampa.sys type=kernel && sc.exe start
    ampa
features:
- pipes-stdin
- pipes-stdout
