...
name: inetcache.sys
description: inetcache.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create inetcache binPath=C:\windows\temp\inetcache.sys type=kernel
    && sc.exe start inetcache
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create inetcache binPath=C:\windows\temp\inetcache.sys type=kernel && sc.exe
    start inetcache
references:
- label: Reference
