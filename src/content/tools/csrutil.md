...
execution:
  ...
  shell: false
examples:
- description: 'Disable SIP: disable SIP (System Integrity Protection) - requires
    booting into recovery mode'
  command: csrutil disable
- description: 'Disable authenticated-root: When authenticated-root is disabled, booting
    is allowed from non-sealed system snapshots - requires booting into recovery mode'
  command: csrutil authenticated-root disable
- description: 'Add a netboot server: Insert a new IPv4 address in the list of allowed
    NetBoot sources'
...
- description: 'Determine if SIP is enabled: Determine if System Integrity Protection
    is enabled'
...
- label: 'The XCSSET Malware: Inserts Malicious Code Into Xcode Projects, Performs
    UXSSBackdoor Planting in Safari, and LeveragesTwo Zero-day Exploits'
