...
name: 6c8a.sys
description: 6c8a.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create 6c8a binPath=C:\windows\temp\6c8a.sys type=kernel && sc.exe
    start 6c8a
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create 6c8a binPath=C:\windows\temp\6c8a.sys type=kernel && sc.exe start
    6c8a
references:
- label: Reference
