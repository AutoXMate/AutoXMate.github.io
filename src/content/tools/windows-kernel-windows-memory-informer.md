...
description: Windows-Memory-Informer.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create Windows-Memory-Informer binPath=C:\windows\temp\Windows-Memory-Informer.sys
    type=kernel && sc.exe start Windows-Memory-Informer
  sandbox: execFile
  timeout_seconds: 30
  shell: true
install:
- method: custom
  ...
  commands:
  - sc.exe create Windows-Memory-Informer binPath=C:\windows\temp\Windows-Memory-Informer.sys
    type=kernel && sc.exe start Windows-Memory-Informer
references:
- label: Reference
