...
execution:
  template: sc.exe create RtsPer.sys binPath=C:\windows\temp\RtsPer.sys type=kernel
...
- method: custom
  ...
  commands:
  - sc.exe create RtsPer.sys binPath=C:\windows\temp\RtsPer.sys type=kernel && sc.exe
    start RtsPer.sys
references:
- label: Reference
