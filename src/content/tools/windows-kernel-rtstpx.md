...
name: RtsTpx.sys
description: RtsTpx.sys is a vulnerable kernel driver from the KeServiceDescriptorTable/vulnerable-drivers
...
execution:
  template: sc.exe create RtsTpx binPath=C:\windows\temp\RtsTpx.sys type=kernel &&
    sc.exe start RtsTpx
  sandbox: execFile
  timeout_seconds: 30
  shell: true
...
- method: custom
  ...
  commands:
  - sc.exe create RtsTpx binPath=C:\windows\temp\RtsTpx.sys type=kernel && sc.exe
    start RtsTpx
references:
- label: Reference
