...
// --- Flag/Format Inference Engine ---
function inferFlag(aliases) {
  if (!aliases || aliases.length === 0) return "";
  const sorted = [...aliases].sort((a, b) => a.length - b.length);
  return sorted[0].split(/[ <=\[{]/)[0].trim();
}

function inferFormat(aliases) {
  if (!aliases) return "space";
  for (const a of aliases) {
    if (a.includes("=")) return "equals";
    if (/ <[\w\-.]+>$/.test(a)) return "space";
    if (/ \[[\w\-.]+\]$/.test(a)) return "space";
  }
  return "space";
}

function annotateParameters(parameters) {
  if (!parameters) return parameters;
  return parameters.map((p) => {
    const flag = p.flag || inferFlag(p.aliases);
    const format = p.format || inferFormat(p.aliases);
    const result = { ...p };
    if (flag) result.flag = flag;
    if (format !== "space" || p.format) result.format = format;
    if (p.positional_order) result.positional_order = p.positional_order;
    return result;
  });
}
...
async function buildApi() {
  ...
  for await (const filePath of glob(pattern, { cwd: srcDir, root: srcDir })) {
    ...
    try {
      ...
      if (entry.parameters) {
        entry.parameters = annotateParameters(entry.parameters);
      }
  ...
  try {
    const queriesScript = path.resolve("src/scripts/build-queries.js");
    if (fs.existsSync(queriesScript)) {
      execSync(`node "${queriesScript}"`, { stdio: "inherit", cwd: path.resolve(".") });
    }
  } catch (e) {
    console.warn("  Queries index generation skipped:", e.message);
  }
